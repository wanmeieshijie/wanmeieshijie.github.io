...
| 📱 响应式布局 | 手机、平板、电脑自适应 |
| 🌗 深色/浅色模式 | 一键切换，自动记忆偏好 |
| ❄️ 雪景动画 | Canvas 2D 渲染，高 DPI 适配，流畅滚动 |
...
- 雪花颜色为纯白色（`rgba(255,255,255,透明度)`），透明度随机 0.5~1。
- Canvas 2D 渲染，自动适配高 DPI 屏幕，滚动流畅不卡顿。

## 技术栈

- **HTML5 + CSS3** — 语义化结构 + CSS 变量主题系统
- **原生 JavaScript** — 零依赖，性能轻量
- **Canvas 2D + requestAnimationFrame** — ❄️ 雪花动画渲染（高 DPI 适配）
- **GitHub Pages** — 静态托管
